...
def main_decryption_program():
    cipher_text_block = input("Enter the cipher text Block (4 hex digits): ")
    ...
    # print(f"ShiftRow 1: {matrix_to_block_hex(shiftrow1)}")

    # Adding round key K2
    k2_added_matrix = XOR_matrices(shiftrow1, K2)
    # print(f"After K2 added: {matrix_to_block_hex(k2_added_matrix)}")

    # Making subnibbles
    subnibbles1 = inv_sub_nibbles(k2_added_matrix)
    # print(f"Subnibbles 1: {matrix_to_block_hex(subnibbles1)}")

    # Shifting Row again
    shiftrow2 = shift_row(subnibbles1)
    # print(f"ShiftRow 2: {matrix_to_block_hex(shiftrow2)}")

    # Calculating mix columns
    mixcolumn_matrix = mix_columns(shiftrow2)
    # print(f"Mix Column: {matrix_to_block_hex(mixcolumn_matrix)}")

    # Adding round key K1
    k1_added_matrix = XOR_matrices(mixcolumn_matrix, K1)
    # print(f"After K1 added: {matrix_to_block_hex(k1_added_matrix)}")

    # Making subnibbles again for the final step
    plain_text = inv_sub_nibbles(k1_added_matrix)  
    print(f"Decrypted Block: {matrix_to_block_hex(plain_text)}")
